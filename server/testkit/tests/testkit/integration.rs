...
async fn test_domain_reset_token_key(rsclient: &KubidmClient) {
...
async fn test_all_persons_has_builtin_class(rsclient: &KubidmClient) {
    login_put_admin_idm_admins(rsclient).await;
    let res = rsclient
...
// fn test_cmd_admin(token_cache_path: &str, rsclient: &KubidmClient, cmd: &str) -> Output {
//     let split_cmd: Vec<&str> = cmd.split_ascii_whitespace().collect();
//     test_cmd_admin_split(token_cache_path, rsclient, &split_cmd)
// }
// /// run a test command as the admin user
// fn test_cmd_admin_split(token_cache_path: &str, rsclient: &KubidmClient, cmd: &[&str]) -> Output {
//     println!(
//         "##################################\nrunning {}\n##################################",
//         cmd.join(" ")
//     );
...
async fn test_integration_with_assert_cmd(rsclient: KubidmClient) {
