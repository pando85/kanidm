...
const commit = process.env.GUIDE_REVIEW_COMMIT || execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
        ...
        for (const motion of motions) {
            for (const story of stories) {
                ...
                const context = await browser.newContext({
                    ...
                    recordVideo: story === "success" || story === "method-choice" ? { dir: artifactDir } : undefined,
                });
                const page = await context.newPage();
                await page.addInitScript(() => {
                    globalThis.__kubidmEvidenceTrace = [];
                    window.addEventListener("kubidm:guide-state", (event) => {
                        globalThis.__kubidmEvidenceTrace.push({ type: "semantic", at: performance.now(), detail: event.detail });
                    });
                    window.addEventListener("kubidm:guide-diagnostics", (event) => {
                        globalThis.__kubidmEvidenceTrace.push({ type: "diagnostic", at: performance.now(), detail: event.detail });
                    });
                });
                ...
                page.on("requestfailed", (request) => {
                    networkEntries.push({ url: request.url(), error: request.failure()?.errorText || "request failed" });
                });

                await page.goto(urlFor(story, theme, viewportName, motion), { waitUntil: "networkidle" });
                if (motion === "full") {
                    await page.waitForFunction(
                        (expectedMode) => {
                            ...
                            return expectedMode === "mock"
                                ...
                                : diagnostic?.loaded === true && diagnostic?.renderer === "rive" && diagnostic?.mockRuntime !== true;
                        },
                        mode,
                        { timeout: 10_000 },
                    );
                ...
                await page.screenshot({ path: path.join(artifactDir, "start.png"), fullPage: true });
                await page.waitForTimeout(350);
                await page.screenshot({ path: path.join(artifactDir, "mid.png"), fullPage: true });
                await page.waitForTimeout(750);
                await page.screenshot({ path: path.join(artifactDir, "end.png"), fullPage: true });
                ...
                manifest.runtimeMetadata ||= diagnostic;
                ...
                allConsole.push({ story, viewport: viewportName, theme, motion, entries: consoleEntries });
                allNetwork.push({ story, viewport: viewportName, theme, motion, entries: networkEntries });
                await context.close();
            }
        }
    }
