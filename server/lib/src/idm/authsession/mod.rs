...
mod tests {
    ...
    use kubidm_proto::internal::{UatPurpose, UserAuthToken};
    use kubidm_proto::oauth2::{AccessTokenResponse, AccessTokenType, IssuedTokenType};
    use kubidm_proto::v1::{AuthAllowed, AuthIssueSession, AuthMech};
    use std::time::Duration;
    use time::OffsetDateTime;
    ...
    fn setup_webauthn_passkey(
        ...
        let r = wa
            .perform_register(webauthn.get_allowed_origins()[0].clone(), chal, 60000)
    ...
    fn test_idm_authsession_webauthn_only_mech() {
        ...
        let (webauthn, mut wa, wan_cred) = setup_webauthn_passkey(account.spn());
        ...
        // Check good challenge
        {
            ...
            let resp = wa
                .perform_auth(webauthn.get_allowed_origins()[0].clone(), chal, 60000)
                .map(Box::new)
        ...
        // Check bad challenge.
        {
            ...
            let resp = wa
                ...
                .perform_auth(webauthn.get_allowed_origins()[0].clone(), inv_chal, 60000)
                .map(Box::new)
                .expect("failed to use softtoken to authenticate");
    ...
    fn test_idm_authsession_webauthn_password_mech() {
        ...
        let (webauthn, mut wa, wan_cred) = setup_webauthn_securitykey(account.spn());
        let pw_good = "test_password";
        let pw_bad = "bad_password";
