...
Kubidm ships with a secure-by-default configuration, however that is only as strong as the environment that Kubidm
operates in. This means the security of your container environment and server is extremely important when running
Kubidm.

This chapter will detail a number of warnings and security practices you should follow to ensure that Kubidm operates in
a secure environment.

The main server is a high-value target for a potential attack, as Kubidm serves as the authority on identity and
authorisation in a network. Compromise of the Kubidm server is equivalent to a full-network take over, also known as
"game over".
...
For this reason, Kubidm's components must be secured and audited. Kubidm avoids many classic attacks by being developed
in a memory safe language, but risks still exist in the operating environment.

## Startup Warnings

At startup Kubidm will warn you if the environment it is running in is suspicious or has risks. For example:

```bash
kubidmd server -c /tmp/server.toml
...
Files, such as configuration files, should be read-only to the UID of the Kubidm daemon. If an attacker is able to gain
...
this file. This may mean that if an account on the system is compromised the attacker can read Kubidm content and may be
...
This can be prevent by making the file owner a different UID than the running process for kubidm.

### A Secure Example

Between these three issues it can be hard to see a possible strategy to secure files, however one way exists - group
read permissions. The most effective method to secure resources for Kubidm is to set configurations to:

```bash
[william@amethyst 12:26] /etc/kubidm > ls -al server.toml
-r--r-----   1 root           kubidm      212 28 Jul 16:53 server.toml
```

The Kubidm server should be run as "kubidm:kubidm" with the appropriate user and user private group created on your
system. This applies to unixd configuration as well.

For the database your data folder should be:

```bash
[root@amethyst 12:38] /data/kubidm > ls -al .
total 1064
drwxrwx---   3 root     kubidm      96 29 Jul 12:38 .
-rw-r-----   1 kubidm   kubidm  544768 29 Jul 12:38 kubidm.db
```

This means 770 root:kubidm. This allows Kubidm to create new files in the folder, but prevents Kubidm from being able to
...
Configurations for clients, such as /etc/kubidm/config, should be secured with read-only permissions and owned by root:

```bash
[william@amethyst 12:26] /etc/kubidm > ls -al config
...
The commands provided in this book will run kubidmd as "root" in the container to make the onboarding smoother. However,
...
You will need to adjust the permissions on the `/data` volume to ensure that the process can manage the files. Kubidm
...
chmod 750 /data/db/
sed -i -e "s/db_path.*/db_path = \"\/data\/db\/kubidm.db\"/g" /data/server.toml
...
You can then use this to run the Kubidm server in docker with a user:

```bash
docker run --rm -i -t -u 1000:1000 -v kubidmd:/data kubidm/server:latest /sbin/kubidmd ...
```
