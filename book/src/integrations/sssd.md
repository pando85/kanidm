...
> [!WARNING]
>
> SSSD should be considered a "last resort". If possible, always use the native Kubidm pam and nsswitch tools instead.

## Limitations

SSSD has many significant limitations compared to Kubidm's native [PAM and nsswitch](./pam_and_nsswitch.md) provider.

### Performance

Kubidm's native provider outperforms SSSD significantly for both online and offline user resolving and operations.
Because of this, SSSD can cause higher load on the Kubidm server due to its design limitations.

### Features

SSSD is not able to access all of the features of Kubidm, limiting the integration options available to you.

### Security

By default Kubidm uses state of the art cryptographic methods with configurable TPM binding of cached local credentials.
...
In addition, Kubidm's providers are written in Rust rather than C, meaning they have less surface area for attack and
compromise. These providers have been through multiple security audits performed by the SUSE product security teams.

## Support

If you choose to use the SSSD provider the Kubidm project will only provide "best effort" for compatibility and issue
resolution.
...
```toml
# Example configuration for SSSD to resolve accounts via Kubidm
#
# This should always be a "last resort". If possible you should always use the
# kubidm pam and nsswitch resolver as these will give you a better and more
# reliable setup.
#
...
# Your URI must be LDAPS. Kubidm does not support StartTLS.
ldap_uri = ldaps://idm.example.com

# These allow SSSD to resolve user primary groups, which in Kubidm are implied by
...
# can't login.
#
# Note that because of how Kubidm presents group names, this value SHOULD be an SPN
ldap_access_filter = (memberof=idm_all_accounts@idm.example.com)

# Set the home dir override. Kubidm does not support configuration of homedirs as an
