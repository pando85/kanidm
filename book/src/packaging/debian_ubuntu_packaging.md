...
- Debian packaging is complex enough that it lives in a separate repository:
  [kubidm/kubidm_ppa_automation](https://github.com/kubidm/kubidm_ppa_automation).
- While community-maintained packages are available at <https://kubidm.github.io/kubidm_ppa/> these instructions will
...
1. Start in the root directory of the main [kubidm/kubidm](https://github.com/kubidm/kubidm) repository.
2. Pull in the separate deb packaging submodule:

```shell
git submodule update platform/debian/kubidm_ppa_automation
```
...
```shell
platform/debian/kubidm_ppa_automation/scripts/install_ci_build_dependencies.sh
```

5. Launch your desired target build:

```shell
platform/debian/kubidm_ppa_automation/scripts/build_native.sh aarch64-unknown-linux-gnu
```
...
```shell
platform/debian/kubidm_ppa_automation/scripts/build_debs.sh aarch64-unknown-linux-gnu
```
...
- The repo is: [kubidm/kubidm](https://github.com/kubidm/kubidm)
...
### Configuration in the kubidm_ppa_automation repo

- The repo is: [kubidm/kubidm_ppa_automation](https://github.com/kubidm/kubidm_ppa_automation)
...
- Finally, once your changes have been approved go back to the main `kubidm/kubidm` repo and update the submodule
  reference and PR the reference update. This is not needed for official builds but helps anyone doing dev builds
  themselves:

  ```shell
  cd platform/debian/kubidm_ppa_automation
  git pull
  cd -
  git add platform/debian/kubidm_ppa_automation
  git commit -m "Update kubidm_ppa_automation reference to latest"
  ```
