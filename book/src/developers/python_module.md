...
- every class and function is fully python typed (test by running `make test/pykubidm/mypy`)
...
2. Build the base environment. From within the `pykubidm` directory, run: `uv sync` This'll set up a virtual environment
   and install all the required packages (and development-related ones)
3. Start editing!

Most IDEs will be happier if you open the kubidm_rlm_python or pykubidm directories as the base you are working from,
rather than the kubidm repository root, so they can auto-load integrations etc.
