...
Kubidm tokens should have a unique pattern, making them easy to recognize. This is crucial for security systems that aim
...
We can make this compatible with current validators by checking if the submitted token starts with `kubidm_`. If it
...
`kubidm_<CREDENTIAL>`. A simple string match and split is more efficient in this case.
