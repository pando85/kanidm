...
We can not expect every application to update to use the Kubidm specific API's nor any other modern
API we offer, so we should allow these applications to at least be able to read into Kubidm via
an LDAP interface.

A major use case is legacy systems that Kubidm's native unix services aren't available on, so these
would be better served via LDAP - of course, the native kubidm unix integrations are better
...
The majority of value is in a working search and bind operation set. The LDAP gateway to Kubidm
will be Read Only as a result. The ability to write via LDAP will not be supported.

Most LDAP servers offer their schema in a readonly interface. Translating Kubidm's schema into a way
that clients could interpret is of little value as many clients do not request this, or parse it.

While Kubidm is entry based, and NoSQL, similar to LDAP, our datamodel is subtly different enough
...
should it (ie radius_secret, you should use the kubidm radius integration instead of the LDAP
interface, as the kubidm interface is more efficient).
...
required to be addressed in the main Kubidm server core. Due to the architecture in Kubidm, all
...
This is derived from the domain name. All entries are presented in a flat scope, the same as Kubidm
...
| Kubidm Attribute  | LDAP Attribute    |
| ----------------- | ----------------- |
| name              | cn                |
| spn               | uid               |
| gidnumber         | uidnumber         |
...
We will accept (and prefer) that Kubidm attribute names are provided in the LDAP filter for applications
that can be customised.

Compatibility Attributes
========================

Some attributes exist in LDAP that have no direct equivalent in Kubidm. These are often from existing
...
These should be provided through an ldapCompat class in kubidm, and require no other transformation. They
may require generation from the server, as legacy applications expect their existence and kubidm created
...
member/memberOf: Member in LDAP is a DN, where in Kubidm it's a reference type with SPN. We will need
to transform this in filters *and* in entries that are sent back.

gecos: This needs synthesisation from displayname

homeDirectory: This needs to match the rules in kubidm_unix_int, especially once trusts are added (likely to be uuid based).

Kubidm is closest to rfc2307bis due to the structure of its memberOf attributes and other
elements, so this is what we will "look like" but may not strictly conform to.


Kubidm Required Changes (TODO list)
===================================
