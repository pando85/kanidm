...
It's important before you start trying to write code and contribute that you understand what Kubidm does and its goals.
...
Each operating system has different steps required to configure and build Kubidm.
...
> NOTE: clang and lld are required to build Kubidm due to performance issues with GCC/ld
...
Kubidm is (largely) a monorepo. This can be checked out with:
...
Other supporting projects can be found on the [project github](https://github.com/kubidm)

If you are forking, then fork in GitHub and then add your remote.

```bash
git remote add myfork git@github.com:<YOUR USERNAME>/kubidm.git
```
...
> [!IMPORTANT]
>
> Kubidm is unable to accept code that is generated by an AI for legal reasons. copilot and other tools that generate
> code in this way can not be used in Kubidm.
...
A list of links to the library documentation is at [kubidm.com/documentation](https://kubidm.com/documentation/).
...
Build profiles allow us to change the operation of Kubidm during it's compilation for development or release on various
platforms. By default the "developer" profile is used that assumes the correct relative paths within the monorepo.

Setting different developer profiles while building is done by setting the environment variable `KUBIDM_BUILD_PROFILE`
to one of the bare filename of the TOML files in `/profiles`.

For example, this will set the CPU flags to "none" and the location for the Web UI files to `/usr/share/kubidm/ui/pkg`:

```bash
KUBIDM_BUILD_PROFILE=release_linux cargo build --release --bin kubidmd
```
...
We recommend using [Let's Encrypt](https://letsencrypt.org), but if this is not possible kubidmd will create self-signed
certificates in `/tmp/kubidm`.

You can now build and run the server with the commands below. It will use a database in `/tmp/kubidm/kubidm.db`.
...
```bash
cargo run --bin kubidm -- --help
cargo run --bin kubidm -- login -H https://localhost:8443 -D anonymous -C /tmp/kubidm/ca.pem
cargo run --bin kubidm -- self whoami -H https://localhost:8443 -D anonymous -C /tmp/kubidm/ca.pem

cargo run --bin kubidm -- login -H https://localhost:8443 -D admin -C /tmp/kubidm/ca.pem
cargo run --bin kubidm -- self whoami -H https://localhost:8443 -D admin -C /tmp/kubidm/ca.pem
```

You may find it easier to modify `~/.config/kubidm` per the [book client tools section](../client_tools.md) for extended
...
kubidm raw create -H https://localhost:8443 -C ../insecure/ca.pem -D admin example.create.account.json
kubidm raw create  -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin example.create.group.json

# Apply a json stateful modification to all entries matching a filter
kubidm raw modify -H https://localhost:8443 -C ../insecure/ca.pem -D admin '{"or": [ {"eq": ["name", "idm_person_account_create_priv"]}, {"eq": ["name", "idm_service_account_create_priv"]}, {"eq": ["name", "idm_account_write_priv"]}, {"eq": ["name", "idm_group_write_priv"]}, {"eq": ["name", "idm_people_write_priv"]}, {"eq": ["name", "idm_group_create_priv"]} ]}' example.modify.idm_admin.json
kubidm raw modify -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin '{"eq": ["name", "idm_admins"]}' example.modify.idm_admin.json

# Search and show the database representations
kubidm raw search -H https://localhost:8443 -C ../insecure/ca.pem -D admin '{"eq": ["name", "idm_admin"]}'

# Delete all entries matching a filter
kubidm raw delete -H https://localhost:8443 -C ../insecure/ca.pem -D idm_admin '{"eq": ["name", "test_account_delete_me"]}'
```

### Build a Kubidm Container
...
| ---------------------- | --------------------------------------------------------- | ------------------------- |
| `IMAGE_BASE`           | Base location of the container image.                     | `kubidm`                  |
...
Build a `kubidm` container using `podman`:

```bash
CONTAINER_TOOL=podman make build/kubidmd
```

Build a `kubidm` container and use a redis build cache:

```bash
CONTAINER_BUILD_ARGS='--build-arg "SCCACHE_REDIS=redis://redis.dev.blackhats.net.au:6379"' make build/kubidmd
```
...
Whenever code is merged with the `master` branch of Kubidm, containers are automatically built for `kubidmd` and
`radius`. Sometimes they fail to build, but we'll try to keep them available.

To find information on the packages,
[visit the Kubidm packages page](https://github.com/orgs/kubidm/packages?repo_name=kubidm).
...
```bash
docker pull ghcr.io/kubidm/radius:devel
docker run --rm -it \
    -v $(pwd)/kubidm:/data/kubidm \
    ghcr.io/kubidm/radius:devel
```

This assumes you have a `kubidm` client configuration file in the current working directory.
