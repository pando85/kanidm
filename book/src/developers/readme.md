...
```bash
KANIDM_BUILD_PROFILE=release_linux cargo build --release --bin kubidmd
```
...
Whenever code is merged with the `master` branch of Kanidm, containers are automatically built for `kubidmd` and
`radius`. Sometimes they fail to build, but we'll try to keep them available.

To find information on the packages,
[visit the Kanidm packages page](https://github.com/orgs/kubidm/packages?repo_name=kubidm).
