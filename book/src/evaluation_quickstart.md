...
This section will guide you through a quick setup of Kubidm for evaluation. It's recommended that for a production
...
```bash
docker pull docker.io/kubidm/server:latest
```
...
docker create --name kubidmd \
  -p '443:8443' \
  -p '636:3636' \
  -v kubidmd:/data \
  docker.io/kubidm/server:latest
```

## Copy the configuration to the container

```bash
docker cp server.toml kubidmd:/data/server.toml
```
...
docker run --rm -i -t -v kubidmd:/data \
  docker.io/kubidm/server:latest \
  kubidmd cert-generate
```

## Start Kubidmd Container

```bash
docker start kubidmd
```

## Recover the Admin Role Passwords

The `admin` account is used to configure Kubidm itself.

```bash
docker exec -i -t kubidmd \
...
Kubidm requires locally installed client tools on the system used for administration via the command line. Follow the
...
After installing the `kubidm` tool, you will need to create a configuration file to continue.
...
```shell
kubidm person create <your username> <Your Displayname>
```

## Set up your account credentials

```shell
kubidm person credential create-reset-token <your username>
```
...
- [Replication](repl/), if one Kubidm instance isn't enough
