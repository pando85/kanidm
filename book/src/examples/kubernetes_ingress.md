...
4. Create a file called `k8s.kubidm-nginx-auth-example.yaml` with the block below. Replace every `<string>` (drop the
   ...
   5. `<OAUTH2_RS_BASIC_SECRET>`: Please see the output from step 2.1 or [get](../integrations/oauth2.md) the OAuth2
      resource you create from that step.
   ...
   - [OAuth2 Proxy](https://oauth2-proxy.github.io/oauth2-proxy/) - A OAuth2 proxy is used as an OAuth2 client with
     NGINX
...
6. Check your deployment succeeded by running the following commands:
   ...
   documentation's [troubleshooting page](https://kubernetes.github.io/ingress-nginx/troubleshooting/). If there are
   certificate errors see the CertManger documentation's
   [troubleshooting page](https://cert-manager.io/docs/faq/troubleshooting/).

   Once it has finished deploying, you will be able to access it at `https://<FQDN>` which will prompt you for
   authentication.
