...
kubidm service-account create demo_service "Demonstration Service" demo_group --name idm_admin
kubidm service-account get demo_service --name idm_admin
```
...
```bash
kubidm service-account api-token status --name ENTRY_MANAGER ACCOUNT_ID
kubidm service-account api-token status --name demo_user demo_service
```
...
```bash
kubidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY]
kubidm service-account api-token generate --name demo_user demo_service "Test Token"
kubidm service-account api-token generate --name demo_user demo_service "Test Token" 2020-09-25T11:22:02+10:00
```

If you wish to issue a token that is able to make changes on behalf of the service account, you must add the `-w` flag
during the generate command. It is recommended you only add `-w` when the API token is performing writes to Kubidm.

```bash
kubidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY] --readwrite
kubidm service-account api-token generate --name demo_user demo_service "Test Token" --readwrite
kubidm service-account api-token generate --name demo_user demo_service "Test Token" 2020-09-25T11:22:02+10:00 --readwrite
```
...
kubidm service-account api-token status --name ENTRY_MANAGER ACCOUNT_ID
kubidm service-account api-token status --name demo_user demo_service
kubidm service-account api-token destroy --name ENTRY_MANAGER ACCOUNT_ID TOKEN_ID
kubidm service-account api-token destroy --name demo_user demo_service 4de2a4e9-e06a-4c5e-8a1b-33f4e7dd5dc7
```

### API Tokens with Kubidm HTTPS/REST API
...
- `subject_token`: the service-account API token (see `kubidm service-account api-token generate`)
...
API Tokens issued by Kubidm are a JWS object that contains metadata about the service account that is being granted
...
fail validation and Kubidm will refuse it.
...
```bash
kubidm service-account api-token generate --name ENTRY_MANAGER ACCOUNT_ID LABEL [EXPIRY] --compact
kubidm service-account api-token generate --name demo_user demo_service "Test Token" --compact
```
