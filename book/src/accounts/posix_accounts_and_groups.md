...
Kubidm has features that enable its accounts and groups to be consumed on POSIX-like machines, such as Linux, FreeBSD or
others. Both service accounts and person accounts can be used on POSIX systems.

## Notes on POSIX Features

Many design decisions have been made in the POSIX features of Kubidm that are intended to make distributed systems
easier to manage and client systems more secure.

### UID and GID Numbers

In Kubidm there is no difference between a UID and a GID number. On most UNIX systems a user will create all files with
...
these values. As a result Kubidm accounts _only_ have a GID number, which is also considered to be its UID number as
...
Kubidm does neither of these. As the GID number of the user must be unique, and a user implies the UPG must exist, we
...
Kubidm has asynchronous replication as a feature between database servers. In this case, we need to be able to allocate
...
| kubidm dyn alloc | 1879048192 | 2147483647 |
...
`1000` through `65535` for OS/Distro purposes, and allows Kubidm to continue dynamic allocation in the range
...
```bash
kubidm [person OR service-account] posix set --name idm_admin <account_id> [--shell SHELL --gidnumber GID]

kubidm person posix set --name idm_admin demo_user
kubidm person posix set --name idm_admin demo_user --shell /bin/zsh
kubidm person posix set --name idm_admin demo_user --gidnumber 2001

kubidm service-account posix set --name idm_admin demo_account
kubidm service-account posix set --name idm_admin demo_account --shell /bin/zsh
kubidm service-account posix set --name idm_admin demo_account --gidnumber 2001
```

You can view the accounts POSIX token details with:

```bash
kubidm person posix show --name anonymous demo_user
kubidm service-account posix show --name anonymous demo_account
```
...
```bash
kubidm group posix set --name idm_admin <group_id> [--gidnumber GID]
kubidm group posix set --name idm_admin demo_group
kubidm group posix set --name idm_admin demo_group --gidnumber 2001
```

You can view the accounts POSIX token details with:

```bash
kubidm group posix show --name anonymous demo_group
```
...
Due to the way that Podman operates, in some cases using the Kubidm client inside non-root containers with Kubidm
