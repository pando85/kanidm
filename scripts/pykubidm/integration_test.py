...
# so we can load kubidm without building virtualenvs
sys.path.append("./pykubidm")

from kubidm import KubidmClient


def recover_account(username: str) -> str:
    """runs the kubidmd binary to recover creds"""
    recover_cmd = [
        "cargo",
        "run",
        "--bin",
...
async def main() -> None:
    ...
    # login time!
    admin_client = KubidmClient(uri=host, ca_path="/tmp/kubidm/ca.pem")
    ...
    idm_admin_client = KubidmClient(uri=host, ca_path="/tmp/kubidm/ca.pem")
