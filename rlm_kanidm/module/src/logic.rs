...
impl Module {
    pub async fn from_config(cfg: KanidmRadiusConfig) -> Result<Self, ModuleError> {
        ...
        let mut client_builder = KubidmClientBuilder::new()
            ...
            client_builder = client_builder
                ...
                .map_err(|e| {
                    ModuleError::Io(format!(
                        "Failed loading ca_path {ca_path} into KubidmClientBuilder: {e:?}"
                    ))
                })?;
        }
        ...
        let client = client_builder
            .build()
            .map_err(|e| ModuleError::Http(format!("Failed creating KubidmClient: {e:?}")))?;
    ...
    async fn fetch_token(&self, user_id: &str) -> Result<Option<RadiusAuthToken>, ModuleError> {
        ...
        match lookup_result {
            ...
            Err(error) => Err(ModuleError::Http(format!(
                "kubidm_client request failed: {error:?}"
            ))),
        }
    }
}
...
mod tests {
    ...
    async fn setup_radius_service_account(rsclient: &KubidmClient) -> (String, Url) {
        rsclient
    ...
    async fn setup_radius_user_secret(rsclient: &KubidmClient, username: &str) -> String {
        rsclient
