...
def test_load_config_file() -> None:
    ...
    if not Path(EXAMPLE_CONFIG_FILE).expanduser().resolve().exists():
        pytest.skip(f"Can't find client config file {EXAMPLE_CONFIG_FILE}")
...
def test_parse_config_validationerror(client: KubidmClient) -> None:
...
async def test_parse_config_data(client: KubidmClient) -> None:
...
def test_config_invalid_uri() -> None:
    ...
    with pytest.raises(pydantic.ValidationError):
        KubidmClientConfig.model_validate(test_input)
...
def test_config_loader_str() -> None:
    ...
    with pytest.raises(FileNotFoundError):
        KubidmClient(config_file="hello world")


def test_config_init() -> None:
    """tests passing config object"""

    config = KubidmClientConfig(uri="https://idp.crabzrool.test")
    assert KubidmClient(config=config)
