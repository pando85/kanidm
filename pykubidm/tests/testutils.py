...
async def client() -> Optional[KubidmClient]:
    """sets up a client with a basic thing"""
    try:
        basicConfig(level=DEBUG)

        return KubidmClient(uri="https://idm.example.com")
    except FileNotFoundError:
        pytest.skip("Couldn't find config file...")  # type: ignore[call-non-callable]


@pytest.fixture(scope="function")
async def client_configfile() -> Optional[KubidmClient]:
    """sets up a client from a config file"""
    try:
        return KubidmClient(config_file=Path("~/.config/kubidm").expanduser())
    except FileNotFoundError:
        pytest.skip("Couldn't find config file...")  # type: ignore[call-non-callable]
...
@pytest.fixture(scope="function")
async def openapi_api_client(openapi_client: KubidmClient) -> AsyncIterator["ApiClient"]:
...
@pytest.fixture(scope="function")
async def openapi_api_client_authed(openapi_authed_client: KubidmClient) -> AsyncIterator["ApiClient"]:
