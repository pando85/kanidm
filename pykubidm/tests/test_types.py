...
from kubidm.types import AuthInitResponse, KubidmClientConfig, RadiusGroup, RadiusClient
...
def test_kubidmconfig_parse_toml() -> None:
    """tests KubidmClientConfig.parse_toml()"""

    config = KubidmClientConfig()
...
def test_radius_client_bad_hostname() -> None:
    ...
    with pytest.raises(ValidationError):
        RadiusClient(
            name="test",
            ipaddr="thiscannotpossiblywork.kubidm.example.com",
            secret="nothing",
        )

    assert RadiusClient(name="test", ipaddr="kubidm.com", secret="nothing")
