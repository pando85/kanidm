...
from kubidm_openapi_client.exceptions import ApiException
from kubidm.exceptions import AuthBeginFailed, AuthCredFailed, AuthInitFailed
from kubidm.types import ClientResponse


@pytest.mark.asyncio
async def test_status_wrapper_delegates_to_system_api(mocker: Any) -> None:
    client = KubidmClient(uri="https://localhost:8443")
    status_mock = mocker.patch(
        "kubidm_openapi_client.api.system_api.SystemApi.status",
...
async def test_scim_list_wrappers_delegate_to_scim_api(
    ...
    method_mock = mocker.patch(
        f"kubidm_openapi_client.api.scim_api.ScimApi.{api_method_name}",
...
async def test_scim_application_get_wrapper_delegates_to_scim_api(mocker: Any) -> None:
    ...
    sentinel = object()
    method_mock = mocker.patch(
        "kubidm_openapi_client.api.scim_api.ScimApi.scim_application_id_get",
        new=AsyncMock(return_value=sentinel),
    )
...
async def test_check_token_valid_wrapper_delegates_to_auth_api(mocker: Any) -> None:
    client = KubidmClient(uri="https://localhost:8443")
    method_mock = mocker.patch(
        "kubidm_openapi_client.api.auth_api.AuthApi.auth_valid_with_http_info",
...
async def test_check_token_valid_wrapper_syncs_openapi_token_for_default_token(mocker: Any) -> None:
    client = KubidmClient(uri="https://localhost:8443", token="token-from-config")
...
async def test_auth_init_wrapper_delegates_to_auth_api(mocker: Any) -> None:
    ...
    method_mock = mocker.patch(
        "kubidm_openapi_client.api.auth_api.AuthApi.auth_post_with_http_info",
...
async def test_auth_begin_wrapper_delegates_to_auth_api(mocker: Any) -> None:
    ...
    response = mocker.Mock(
        status=200,
        headers={"x-kubidm-auth-session-id": "session-id"},
        read=AsyncMock(return_value=b'{"sessionid":"session-id","state":{"continue":["password"]}}'),
    )
    method_mock = mocker.patch(
        "kubidm_openapi_client.api.auth_api.AuthApi.auth_post_without_preload_content",
        new=AsyncMock(return_value=response),
    )
    ...
    assert kwargs == {"_headers": {"x-kubidm-auth-session-id": "session-id"}}
...
async def test_auth_as_anonymous_uses_openapi_auth_post_compat(mocker: Any) -> None:
    ...
    assert auth_post_mock.await_args.kwargs["headers"] == {"x-kubidm-auth-session-id": "session-id"}
