...
```bash
python -m pip install kubidm
```

## Documentation

Documentation can be generated by [cloning the repository](https://github.com/kubidm/kubidm) and running
`make docs/pykubidm/build`. The documentation will appear in `./pykubidm/site`. You'll need make and the
...
The OpenAPI spec is served by the running kubidm server. To generate/update the OpenAPI-based client package:
...
```bash
uv run kubidm_openapi_codegen --spec-file /path/to/openapi.json
uv run kubidm_openapi_codegen --spec-url https://localhost:8443/docs/v1/openapi.json
```

Programmatic code generation lives in `kubidm.openapi_codegen`, which is behind the optional feature `openapi_codegen`:

```bash
python -m pip install "kubidm[openapi_codegen]"
```
...
generate_openapi_client(
    ...
    verify_tls=True,
    ca_file=Path("/tmp/kubidm/ca.pem"),
    # season to taste
    output=Path("pykubidm/kubidm_openapi_client"),
)
```

The generated client is packaged as `kubidm_openapi_client`.

`KubidmClient` now initializes an OpenAPI client by default at startup as `client.openapi_client`:
...
You can still build one explicitly from config helpers in `kubidm.openapi` if needed.

#### OpenAPI structure

- `kubidm_openapi_client/` contains the generated OpenAPI client (API classes, models, and transport).
- `kubidm/openapi.py` provides helpers to build an OpenAPI `ApiClient` from a `KubidmClient`/`KubidmClientConfig`.
...
OpenAPI tests regenerate `kubidm_openapi_client` once per pytest session before running marked tests. This uses
`kubidm.openapi_codegen` and requires Docker to be available.
