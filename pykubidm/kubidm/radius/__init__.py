...
from kubidm.exceptions import NoMatchingEntries
from kubidm.types import AuthState, RadiusTokenResponse
...
CONFIG_PATHS = [
    ...
    "/data/kubidm",  # fallback to old path
]
...
def instantiate(_: Any) -> Any:
    ...
    if config_path is None:
        ...
        kubidm_client = KubidmClient(config_file=config_path)
...
async def _get_radius_token(
    ...
    response = await kubidm_client.get_radius_token(username=username)
...
def authorize(
    ...
    try:
        ...
        logging.info(
            "kubidm RLM_MODULE_NOTFOUND after NoMatchingEntries for user_id %s: %s",
            user_id,
            error_message,
        )
        return radiusd.RLM_MODULE_NOTFOUND
    except ClientConnectorError as client_error:
        logging.error("kubidm client connector error in http layer: %s", client_error)
        return radiusd.RLM_MODULE_FAIL
    except Exception as error_message:  # pylint: disable=broad-except
        logging.error("kubidm exception: %s, %s", type(error_message), error_message)
        return radiusd.RLM_MODULE_FAIL
    if tok is None:
        logging.info("kubidm RLM_MODULE_REJECT - unable to retrieve radius information token")
    ...
    req_sat = False
    required_groups = kubidm_client.config.radius_required_groups
    ...
    uservlan: int = reduce(
        check_vlan,
        tok.groups,
        kubidm_client.config.radius_default_vlan,
    )
    if uservlan == int(0):
    ...
    reply = (
        ("User-Name", str(name)),
        ("Reply-Message", f"kubidm-Uuid: {uuid}"),
        ("Tunnel-Type", "13"),
...
def authenticate(
    ...
    try:
        loop = asyncio.get_event_loop()
        return loop.run_until_complete(kubidm_client.check_token_valid())
    except Exception as error_message:  # pylint: disable=broad-except
        logging.error("Failed to run kubidm.check_token_valid: %s", error_message)
    return radiusd.RLM_MODULE_FAIL
