"""Generate the OpenAPI client package for pykubidm.
...
DEFAULT_PACKAGE = "kubidm_openapi_client"
...
def _load_docker_module() -> Any:
    try:
        ...
        raise RuntimeError(
            "OpenAPI codegen requires the optional feature 'openapi_codegen'. Install with: pip install 'kubidm[openapi_codegen]'"
        ) from exc
...
def _get_project_version() -> str:
    ...
    try:
        ...
        return importlib.metadata.version("kubidm")
...
def _run_generator(
    ...
    additional_properties = [
        ...
        f"httpUserAgent=kubidm-python/{_get_project_version()}",
    ]
    if library:
    ...
    notice.write_text(
        "\n".join(
            [
                ...
                "Generator: kubidm_openapi_codegen",
                "",
                "To regenerate:",
                "  uv run kubidm_openapi_codegen",
            ]
        )
        + "\n",
        encoding="utf-8",
