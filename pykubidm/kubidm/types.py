...
class AuthBeginResponse(BaseModel):
    ...
    # this should be pulled from the response headers as x-kubidm-auth-session-id
...
class RadiusClient(BaseModel):
    """Client config for kubidm FreeRADIUS integration,
...
class KubidmClientConfig(BaseModel):
    """Configuration file definition for kubidm client config
    Based on struct KubidmClientConfig in kubidm_client/src/lib.rs
