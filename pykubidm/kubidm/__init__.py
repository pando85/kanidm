...
from kubidm_openapi_client.models.entry import Entry as OpenApiEntry
from pydantic import ValidationError
import yarl

from kubidm.models.group import Group, GroupList, RawGroup
from kubidm.models.oauth2_rs import OAuth2Rs, Oauth2RsList, RawOAuth2Rs
from kubidm.models.person import (
    Person,
    PersonList,
    RawPerson,
    PersonCredentialResetToken,
)
from kubidm.models.service_account import (
    ServiceAccount,
    RawServiceAccount,
    ServiceAccountList,
)
...
if TYPE_CHECKING:
    from kubidm_openapi_client.models.scim_entry import ScimEntry as OpenApiScimEntry
    from kubidm_openapi_client.models.scim_list_response import ScimListResponse as OpenApiScimListResponse

K_AUTH_SESSION_ID = "x-kubidm-auth-session-id"

XDG_CACHE_HOME = (
    Path(os.getenv("LOCALAPPDATA", "~/AppData/Local")) / "cache" if platform.system() == "Windows" else Path(os.getenv("XDG_CACHE_HOME", "~/.cache"))
)

TOKEN_PATH = XDG_CACHE_HOME / "kubidm_tokens"


class KubidmClient:
    """kubidm client module

    config: a `KubidmClientConfig` object, if this is set, everything else is ignored
    ...
    def __init__(
        ...
        config: Optional[KubidmClientConfig] = None,
        ...
        if config is not None:
            ...
            self.config = KubidmClientConfig.model_validate(
                {
                    "uri": uri,
    ...
    def _normalise_openapi_data(data: Any) -> Any:
        if isinstance(data, list):
            return [KubidmClient._normalise_openapi_data(item) for item in data]
        if isinstance(data, tuple):
            return tuple(KubidmClient._normalise_openapi_data(item) for item in data)
        if isinstance(data, dict):
            return {key: KubidmClient._normalise_openapi_data(value) for key, value in data.items()}
        if hasattr(data, "to_dict") and callable(getattr(data, "to_dict")):
            return KubidmClient._normalise_openapi_data(data.to_dict())
        return data
    ...
    async def get_radius_token(self, username: str) -> ClientResponse[Any]:
        ...
        if response.status_code == 404:
            opid = self._header_value(response.headers, "x-kubidm-opid") or ""
