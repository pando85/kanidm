...
SCIMATTR_ONE_OF_SCHEMAS = ["bool", "datetime", "float", "int", "str"]

class ScimAttr(BaseModel):
    """
    ScimAttr
    """
    # data type: bool
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ScimAttr.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ScimAttr with oneOf schemas: bool, datetime, float, int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ScimAttr with oneOf schemas: bool, datetime, float, int, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ScimAttr with oneOf schemas: bool, datetime, float, int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ScimAttr with oneOf schemas: bool, datetime, float, int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
