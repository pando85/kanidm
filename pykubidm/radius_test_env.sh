...
# This sets up a Kubidm environment for doing RADIUS testing.
...
IDM_ADMIN=$(../../scripts/run_insecure_dev_server.sh scripting recover-account idm_admin 2>&1 | grep '\"password' | jq -r .password)
if [ -z "${IDM_ADMIN}" ]; then
...
# set up idm admin account
ADMIN=$(../../scripts/run_insecure_dev_server.sh scripting recover-account admin 2>&1 | grep '\"password' | jq -r .password )
...
export KUBIDM_URL="https://localhost:8443"
export KUBIDM_CA_PATH="/tmp/kubidm/ca.pem"

cd ../../ || exit 1

echo "Logging in as admin"
cargo run --bin kubidm -- login --name admin --password "${ADMIN}"

echo "Logging in as idm_admin"
cargo run --bin kubidm -- login --name idm_admin --password "${IDM_ADMIN}"

echo "Creating person 'test'"
cargo run --bin kubidm -- person create test test --name idm_admin

echo "Creating group 'radius_access_allowed'"
cargo run --bin kubidm -- group create radius_access_allowed --name idm_admin
echo "Adding 'test' to group 'radius_access_allowed'"
cargo run --bin kubidm -- group add-members radius_access_allowed test --name idm_admin

echo "Creating radius secret for 'test'"
cargo run --bin kubidm -- person radius generate-secret test --name idm_admin
echo "Showing radius secret for 'test'"
cargo run --bin kubidm -- person radius show-secret test --name idm_admin
...
cargo run --bin kubidm -- service-account create radius_server radius_server --name idm_admin

echo "Setting radius_server to be allowed to be a RADIUS server"
cargo run --bin kubidm group add-members --name admin idm_radius_servers radius_server

echo "Creating API Token for 'radius_server' account"
cargo run --bin kubidm -- service-account  api-token generate radius_server radius --name admin
